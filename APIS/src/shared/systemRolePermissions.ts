 ...
 *
 * New canonical roles (skill.md §ROLE HIERARCHY):
 *   merchant_owner   → level 2, full tenant access (replaces hq_admin / store_owner)
 *   merchant_manager → level 3, cross-branch mgmt (replaces hq_manager)
 *   accountant       → level 3, financial reports only
 *   supervisor       → level 4, branch-scoped ops (replaces branch_admin)
 *   senior_cashier   → level 5, POS + discount + refund
 */
export const SYSTEM_ROLE_PERMISSIONS: Record<string, string[]> = {
    ...
    merchant_owner: [
        ...
        "reports:view", "reports:sales", "reports:inventory", "reports:financial", "reports:staff", "reports:export",
    ...
    hq_manager: [
        // Read-heavy HQ oversight. Must match defaultAccessControl.ts hq_manager exactly.
        "dashboard:view",
        "sales:view",
