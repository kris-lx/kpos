    ...
    try {
        ...
        const staffResult = await db.execute(sql`
            ...
            SELECT DISTINCT ON (u.id)
                u.tenant_id, u.id, s.id, u.branch_id, true, true, false, false, true
            FROM users u
            JOIN stores s ON s.branch_id = u.branch_id
            ...
            ORDER BY u.id, s.is_default DESC
        `);

        const skusFixed = skuResult.rowCount ?? 0;
        const staffFixed = staffResult.rowCount ?? 0;
